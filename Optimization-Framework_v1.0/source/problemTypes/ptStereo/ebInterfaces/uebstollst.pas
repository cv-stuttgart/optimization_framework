...
type
  ...
  TEB_StollST = class(TEvaluationBinary)
...
class procedure TEB_StollST.executeProcess(const cmdLine, workingDir: string;
...
begin
  ...
  try
    ...
    {$WARNINGS Off}
    estimationProcess.CommandLine := 'batchST ' + cmdLine;
    ...
    try
      ...
      try
        ...
        begin
          ...
          // save RMS
          errorValues[0].name  := 'RMS';
          try
            if SysUtils.SScanf(poString[poString.Count-4], 'RMS: %f', [@errorValues[0].value]) = 0 then
            begin
          ...
          try
            if SysUtils.SScanf(poString[poString.Count-3], 'AEE: %f', [@errorValues[1].value]) = 0 then
            begin
              successful           := false;
          ...
          // save RSE
          errorValues[2].name  := 'RSE';
          try
            if SysUtils.SScanf(poString[poString.Count-2], 'RSE: %f', [@errorValues[2].value]) = 0 then
            begin
              successful           := false;
...
class function TEB_StollST.getCmdLineParameters(
...
initialization
  ebClassManager.registerClass('', TEB_StollST);
  ebClassManager.registerClass('stStoll', TEB_StollST);
