...
type
  ...
  TEB_Stoll = class(TEvaluationBinary)
...
class function TEB_Stoll.getCmdLineParameters(chosenParameters: TChosenParameterSet;
  const cmdLineParameters: string; imageSequenceData: TImageSequenceData
  ): string;
var
  s : string;
...
class procedure TEB_Stoll.executeProcess(const cmdLine, workingDir: string;
...
begin
  ...
  try
    ...
    {$WARNINGS Off}
    estimationProcess.CommandLine := 'batchOF ' + cmdLine;
    ...
    try
      ...
      try
        ...
        successful := poString.Count >= 3;
        if successful then
        begin
          SetLength(errorValues, 3);

          // save AEE
          errorValues[0].name  := 'AAE';
          try
            if SysUtils.SScanf(poString[poString.Count-3], 'AAE: %f', [@errorValues[0].value]) = 0 then
            begin
          ...
          // save AAE
          errorValues[1].name  := 'AEE';
          try
            if SysUtils.SScanf(poString[poString.Count-2], 'AEE: %f', [@errorValues[1].value]) = 0 then
            begin
              successful           := false;
          ...
          // save BP
          errorValues[2].name  := 'BP';
          try
            tmp := Copy(poString[poString.Count-1], 0, Length(poString[poString.Count-1]) - 1);
            if SysUtils.SScanf(tmp, 'BP:  %f', [@errorValues[2].value]) = 0 then
            begin
              successful           := false;
...
initialization
  ebClassManager.registerClass('', TEB_Stoll);
  ebClassManager.registerClass('ofStoll', TEB_Stoll);
